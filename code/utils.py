...
def compute_D_loss(predictions, loss_function, label=False):
    D_loss = 0.
    for prediction in predictions:
        if label:
            target_tensor = Variable(torch.full(prediction[0].size(), 0.9).cuda())
        else:
            target_tensor = Variable(torch.zeros(prediction[0].size()).cuda())
        loss = loss_function(prediction[0], target_tensor)
        D_loss += loss
    return D_loss
